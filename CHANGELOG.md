...
- Collection-scoped vector search runs one sqlite-vec KNN statement with the
  ...
  scan does not yield enough in-scope rows (#775, #791, #803). A metadata
  filter goes through the same pre-filter, so a filter that admits more than
  20k vectors no longer returns nothing when enough ineligible vectors sit
  closer to the query.
